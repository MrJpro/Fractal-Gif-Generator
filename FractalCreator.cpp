...
namespace caveofprogramming {
	...
	//prints all the iterations in the historgram

	//adds a zoom to the zoomList, zoom are applied in order
	void FractalCreator::addZoom(const Zoom& zoom) {
		m_zoomList.add(zoom);
	}

	//calls all functions needed to create the bitmap file
	...
	void FractalCreator::calculateIteration() {
		...
		for (int y = 0; y < m_height; y++) {
			for (int x = 0; x < m_width; x++) {
				...
				int iterations = Mandelbrot::getJuliaIterations(coords.first,
					coords.second);
